...
<script type="text/javascript">
	...
	function init(){
		container = new Vue({
			...
			data : {
				...
				currentPage : 0,
				//totalCntComnGrpCod : 0,
				comnGrpCodPagination : '',
				comnDtlCodPagination : '',
			},
			methods : {
				changedSearchKey : function(event){
					container.oname=event.target.value;
				},
				vuefn_fListComnDtlCod : function(currentPage,  grp_cod, grp_cod_nm){
	...
	function fInitFormGrpCod(object) {
		console.log("fInitFormGrpCod object : "+JSON.stringify(object))
		$("#grp_cod").focus();
	...
	function board_search(currentPage) {
        ...
        console.log("board_search resultCallback : "+JSON.stringify(data));
	...
	function fListComnGrpCod(currentPage) {
		...
		var resultCallback = function(data) {
			flistGrpCodResult(data, currentPage);
			console.log("fListComnGrpCod data : "+JSON.stringify(data));
	...
	function flistGrpCodResult(data, currentPage) {
		...
		//swal(data);
		console.log("flistGrpCodResult data : "+JSON.stringify(data));
		...
		container.listComnGrpCodList = data.listComnGrpCodModel;
		//console.log("flistGrpCodResult data.listComnGrpCodModel : "+JSON.stringify(data.listComnGrpCodModel));
		// 총 개수 추출
		
		//var totalCntComnGrpCod = $("#totalCntComnGrpCod").val();
		container.totalCntComnGrpCod = data.totalCntComnGrpCod;
		...
		var paginationHtml = getPaginationHtml(currentPage, data.totalCntComnGrpCod, pageSizeComnGrpCod, pageBlockSizeComnGrpCod, 'fListComnGrpCod');
		console.log("paginationHtml : " + paginationHtml);
		//swal(paginationHtml);
		//$("#comnGrpCodPagination").empty().append( paginationHtml );
		container.comnGrpCodPagination = paginationHtml;
	...
	function flistDtlCodResult(data, currentPage) {
		console.log("flistDtlCodResult data : "+JSON.stringify(data));
		...
		container.comnDtlCodPagination=paginationHtml;
			...
			<ul>
				...
				<li class="contents">
					...
					<div class="content">
						...
						<div class="divComGrpCodList">
							<table class="col">
								...
								<tbody id="listComnGrpCod">
									<tr v-for="(list, index) in listComnGrpCodList">
										<td>{{(index+1)+(container.currentPage-1)*5}}</td>
										<td><a href="" @click.prevent="vuefn_fListComnDtlCod(1,  list.grp_cod, list.grp_cod_nm)">{{list.grp_cod}}</a></td>
										<td>{{list.grp_cod_nm}}</td>
										<td>{{list.grp_cod_eplti}}</td>
										<td>{{list.use_poa}}</td>
										<td>{{list.fst_enlm_dtt}}</td>																																							
										<td>
										<a class="btnType3 color1" href="javascript:fPopModalComnGrpCod('bank_cd');"><span>수정</span></a>
										</td>
									</tr>
								</tbody>
							</table>
						...
						<div class="paging_area"  id="comnGrpCodPagination" v-html="comnGrpCodPagination"> </div>
                        ...
                        <tr style="border: 0px; border-color: blue">
                           ...
                           <td width="50" height="25" style="font-size: 100%; text-align:left; padding-right:25px;">
     	                      <select id="searchKey" name="searchKey" style="width: 150px;" @change="changedSearchKey($event)">
						...
						<div class="paging_area"  id="comnDtlCodPagination" v-html="comnDtlCodPagination"> </div>
